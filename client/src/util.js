...
export const getNodeColor = (weight) => {
  const colors = [
    ...
    "#26C6DA","#26A69A","#66BB6A","#9CCC65","#D4E157","#FFEE58","#FFCA28",
    "#FFA726","#FF7043","#8D6E63","#8D6E63","#78909C",
  ];
...
export const generateNodeWeightMap = (edges, nodeWeightMap) => {
  ...
  edges.forEach((edge) => {
    if (edge.data.source in newNodeWeightMap) newNodeWeightMap[edge.data.source] += 1;
    else newNodeWeightMap[edge.data.source] = 0;
    if (edge.data.target in newNodeWeightMap) newNodeWeightMap[edge.data.target] += 1;
    else newNodeWeightMap[edge.data.target] = 0;
  });
...
export const cleanTuples = (tuples) => {
  ...
  tuples.forEach((tuple) => {
    if (tuple) results.push(tuple.split(","));
  });
  return results;
};
...
export const cleanJSONTuples = (data) => {
  ...
  data.forEach((elem) => {
    const temp = [];
    Object.values(elem).forEach((val) => temp.push(val));
    result.push(temp);
  });
  return result;
};
...
export const tuplesToGraph = (tuples) => {
  ...
  tuples.forEach((tuple) => {
    if (tuple.length === 3 && tuple[2] !== "True") {
      if (!nodeWeightMap.hasOwnProperty(tuple[0])) nodeWeightMap[tuple[0]] = 1;
      else nodeWeightMap[tuple[0]] += 1;
      if (!nodeWeightMap.hasOwnProperty(tuple[2])) nodeWeightMap[tuple[2]] = 1;
      else nodeWeightMap[tuple[2]] += 1;
      if (!nodes.includes(tuple[0])) nodes.push(tuple[0]);
      if (!nodes.includes(tuple[2])) nodes.push(tuple[2]);
...
export const restructureGraph = (graph) => {
  ...
  const edges = graph.edges.map((edge) => ({ data: { id: uuidv4(), ...edge } }));
  return { nodes, edges };
};
